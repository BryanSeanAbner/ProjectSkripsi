...
<body>
    ...
    @php
        $baseHomeUrl = session('base_home_url', '/homepage');
    @endphp

    @if (!isset($hideHeader))
        <header>
            <a href="{{ url($baseHomeUrl) }}" class="header-logo"
                style="display:flex; flex-direction:column; align-items:flex-start;">
                <img src="{{ asset('img/logo.png') }}" alt="ntvnews.id" style="height: 40px; margin-bottom: 2px;">
            </a>
            ...
            <div class="header-actions">
                <form action="{{ url($baseHomeUrl) }}" method="GET"
                    style="display:flex; align-items:center; background:rgba(255,255,255,0.1); border-radius:20px; padding:2px 10px;">
                    <input type="text" name="q" placeholder="Cari Berita..."
                        style="background:transparent; border:none; color:#fff; outline:none; font-size:12px; width:120px;"
                        value="{{ request('q') }}">
                    <button type="submit" style="background:none; border:none; color:#fff; cursor:pointer;"><i
    ...
    @if (!isset($hideFooter))
        <footer>
            <div class="footer-container">
                <div>
                    <img src="{{ asset('img/logo.png') }}" alt="ntvnews.id" style="height: 32px; margin-bottom: 8px;"><br>
        ...
        async function initGlobalLayout() {
            ...
            if (data && data.length > 0) {
                ...
                if (nav) {
                    ...
                    const baseHomeUrl = '{{ url($baseHomeUrl) }}';
                    const rawBaseHomeUrl = '{{ $baseHomeUrl }}';
                    let html = `<a href="${baseHomeUrl}" ${currentPath === rawBaseHomeUrl || currentPath === '/' ? 'class="active"' : ''}>BERANDA</a>`;
                    data.forEach((sec) => {
